...
function createWMSSpec(wmsURL, gName, gShortName, wmsLayers, wmsStyles, wmsFormat, wmsVersion, wmsBgColor, wmsSrs, wmsCopy) {
  ...
  wmsSpec.getTileURL = function (x, y, zoom){
    ...
    var url = this.wmsURL + "reaspect=false&VERSION=" + this.wmsVersion + "&REQUEST=GetMap&LAYERS=" + this.wmsLayers + "&STYLES=" + this.wmsStyles + "&SRS="+ this.wmsSrs + "&BBOX=" + bbox + "&WIDTH=" + ts +"&HEIGHT=" + ts + "&FORMAT=" + this.wmsFormat + "&TRANSPARENT=" + this.wmsTransparent + '&EXCEPTIONS=INIMAGE';
    return url;
   };
...
var SIG="(c) Teleatlas (c) JRC (c) European Commission";
